...
logging.basicConfig(
    filename=constants.LOG_FILENAME_WEB,
...
def sign_up():
    ...
    if results[0] == userUtils.RetCodes.success.value:
        flash(
            "Congratulations!!! '{0}' successfully signed up. Get started by signing in now.".format(
                form.name.data
            ),
            "is-info",
        )
...
def show_jd_all_page():
    ...
    toggles = {
        "client": {
            ...
            if (orderBy == "client" and order == "ASC")
            else "fa fa-arrow-up",
        ...
        "open_date": {
            "arrowToggle": "fa fa-arrow-down"
            if (orderBy == "open_date" and order == "ASC")
            else "fa fa-arrow-up",
            "orderToggle": "DESC" if order == "ASC" else "ASC",
        },
        "hiring_mgr_name": {
            "arrowToggle": "fa fa-arrow-down"
            if (orderBy == "hiring_mgr_name" and order == "ASC")
            else "fa fa-arrow-up",
            "orderToggle": "DESC" if order == "ASC" else "ASC",
        },
    }
...
def save_JD():
    ...
    results = jdUtils.save_jd(
        form.id.data,
        form.title.data,
        form.details.data,
        form.client.data,
        int(loggedInUserID),
        int(form.total_positions.data),
        form.open_date.data,
        form.intv_panel_name_1.data,
        form.intv_panel_email_1.data,
        form.intv_panel_phone_1.data,
        form.intv_panel_name_2.data,
        form.intv_panel_email_2.data,
        form.intv_panel_phone_2.data,
        form.hiring_mgr_name.data,
        form.hiring_mgr_email.data,
        form.hiring_mgr_phone.data,
        form.hr_name.data,
        form.hr_email.data,
        form.hr_phone.data,
        int(form.status.data),
        form.location.data,
        form.min_yrs_of_exp.data,
        filename,
        form.primary_skills.data,
        form.secondary_skills.data,
        form.ctc_min.data,
        form.ctc_max.data,
        form.ctc_currency.data,
        form.fees_percent.data,
        form.warranty_in_months.data,
    ...
    if results[0] == jdUtils.RetCodes.success.value:
        flash(
            "Congratulations!!! Job Requistion with title '{0}' successfully created".format(
                form.title.data
            ),
            "is-info",
        )
...
def apply_to_JD():
    ...
    _logger.debug(
        "candidate resume file name is {0}".format(form.candidate_resume.data)
    )
    ...
    if form.validate_on_submit():
        ...
        resumeUtils.save_resume(
            constants.NEW_ENTITY_ID,
            filename,
            form.candidate_name.data,
            form.candidate_email.data,
...
def resume_save():
    ...
    (retCode, msg, data) = resumeUtils.save_resume(
        form.id.data,
        filename,
        form.candidate_name.data,
        form.candidate_email.data,
        form.candidate_phone.data,
        int(session.get("user_id")),
    )
...
def search_non_shortlisted_resumes():
    ...
    _logger.debug(
        "Job ID is %s & search criteria is %s", form.job_id.data, form.ft_search.data
    )

    (retCode, msg, resumeList) = jdUtils.get_resumes_not_associated_with_job(
        form.job_id.data, form.ft_search.data, session.get("tenant_id")
    )
    ...
    if retCode == jdUtils.RetCodes.success.value:
        ...
        return render_template(
            "/jd/non_shortlisted_candidates_list.html",
...
def resume_download(id):
    ...
    try:
        ...
        assert (
            code == resumeUtils.RetCodes.success.value
        ), "Failed to fetch resume {0} ".format(id)
...
def jd_download(id):
    ...
    try:
        ...
        assert (
            code == jdUtils.RetCodes.success.value
        ), "Failed to fetch job {0} ".format(id)
...
def show_resume_shortlist_page():
	...
	try:
		...
		assert request.args.get(
		    'name'), "Candidate Name request parameter not found."
		...
		if (results[0] == jdUtils.RetCodes.success.value):
			...
			for jd in jdList:
				_logger.debug(jd.title)
			form.selected_jd_list.choices = [
			    (jd.id, jd.title + " | " + str(jd.id)) for jd in jdList]
...
def show_shortlist_resumes_page(job_id):
    ...
    assert (
        retCode == jdUtils.RetCodes.success.value
    ), "Failed to fetch resumes not associated with job  id {0}. Error code is {1}. Error message is {2}".format(
        job_id, retCode, msg
    )
...
def jd_resume_shortlist():
    ...
    (retCode, msg, data) = jdUtils.shortlist(
        resume_id,
        jd_id,
        datetime.now(tz=timezone.utc),
        resumeUtils.ApplicationStatusCodes.shortlisted.value,
        session.get("user_id"),
    )
    ...
    if retCode == jdUtils.RetCodes.success.value:
        ...
        flash(
            "Candidate shortlisting failed. Error details are as under - "
            + retCode
            + ":"
            + msg,
            "is-danger",
...
def show_job_application_update_page():
    ...
    assert (
        "resume_id" in request.args
    ), "Query parameter {0} not found in request".format("resume_id")
    resume_id = request.args.get("resume_id")
    ...
    _logger.debug(
        "inside show Job application update page for Job ID {0}, resume ID {1} ".format(
            job_id, resume_id
        )
    )
    ...
    assert (
        retCode == resumeUtils.RetCodes.success.value
    ), "Failed to fetch resume associated with resume  id {0}. Error code is {1}. Error message is {2}".format(
    ...
    assert (
        retCode == resumeUtils.RetCodes.success.value
    ), "Failed to get application status codes. Error code is {0} & error message is {1}".format(
        retCode, msg
    )
...
def update_job_application_status():
    ...
    (retCode, msg, retData) = jdUtils.insert_job_application_status(
        form.job_id.data,
        form.resume_id.data,
        form.change_date.data,
        session.get("user_id"),
        form.new_status.data,
        form.notes.data,
    )
    ...
    if retCode == jdUtils.RetCodes.success.value:
        ...
        body = template.render(
            resumeID=str(form.resume_id.data),
            jobID=str(form.job_id.data),
            new_status=str(form.new_status.data),
        )
        # send email
        ...
        flash(
            "Status update failed. Failure detail as follow - " + retCode + ":" + msg,
            "is-danger",
        )
...
def show_job_summary_page(job_id):
    ...
    assert (
        retCode == jdUtils.RetCodes.success.value
    ), "Failed to fetch job details for job_id {0)".format(job_id)

    # get the summary
    (retCode, msg, jobStatusSummary) = jdUtils.get_job_status_summary(job_id)
    assert (
        retCode == jdUtils.RetCodes.success.value
    ), "Failed to fetch job status summary for job_id {0)".format(job_id)

    return render_template(
        "jd/show_status_summary_page.html", jd=jd, statusSummary=jobStatusSummary
    )
...
def show_clientwise_summary_report_page():
    ...
    assert (
        retCode == reports.RetCodes.success.value
    ), "Failed to fetch client wise summary report"

    return render_template(
        "reports/show_clientwise_summary_reportpage.html", clientSummary=clientSummary
    )
...
def show_clientwise_job_application_status_summary_report_page():
    ...
    (
        ...
        msg,
        clientSummary,
    ) = reports.get_client_wise_job_application_status_summary_report(
        session["tenant_id"]
    )
    assert (
        retCode == reports.RetCodes.success.value
    ), "Failed to fetch client wise job application summary report"

    return render_template(
        "reports/show_clientwise_job_app_summary_reportpage.html",
...
def show_clientwise_revenue_opportunity_summary_report_page():
    ...
    return render_template(
        "reports/show_clientwise_revenue_opportunity_reportpage.html",
        clientSummary=clientSummary,
    )
...
def do_reset_password():
    ...
    if form.current_password.data == form.new_password.data:
        ...
        (retCode, msg, data) = userUtils.do_reset_password(
            form.id.data,
            form.email.data,
...
@app.route('/user/showSigninPage/forgotPassword', methods=['GET', 'POST'])
def do_forgot_password_task():
    email = request.form.get('email')
    # print(request.args)
    # print(email)
    # email = 'shelkeniteesh2001@gmail.com'
    user = userUtils.get_user_by_email(email)
    # print(user[2])
    if not user[2]:
        flash('this email is not registerd with us')
        return redirect(url_for('show_signin_page'))
    else:
        new_password = 'WeRecruit2022$'
        userUtils.do_reset_password(
            user[2].id, user[2].email, user[2].password, new_password, type='forgot')
...
def show_add_user_page():
    ...
    form.role.choices = [
        (userUtils.RoleIDs.ADMIN.value, "Admin"),
...
def save_user():
    ...
    (retcode, msg, data) = userUtils.save_user(
        session["tenant_id"],
        form.user_id.data,
        form.name.data,
        form.email.data,
