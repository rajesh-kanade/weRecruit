    ...
    try:
        ...
        if user_attrs is None or len(user_attrs) <= 0:
            return (
                ...
                "user attributes dictionary missing.",
                None,
            )
    ...
    try:
        ...
        if int(userID) == constants.NEW_ENTITY_ID:
            ...
            return (
                ...
                "User {0} added successful.".format(userID),
                userID,
            )
            ...
            return (
                RetCodes.success.value,
                "User {0} record updated successful.".format(userID),
                userID,
            )
...
def get(id):
    try:
        ...
        return (
            RetCodes.success.value,
            "User info for {0} successfully fetched from db".format(id),
            user,
        )
...
def delete(userID):
    try:
        ...
        if int(updated_rows) != 1:
            return (
                RetCodes.del_ent_error.value,
                "DELETE for User ID '{0}' failed. No. of rows updated was {1} instead of 1".format(
                    userID, updated_rows
                ),
                updated_rows,
            )
            ...
            return (
                RetCodes.success.value,
                "DELETE for User ID '{0}' succeeded.".format(userID),
                updated_rows,
            )
...
def get_user_by_email(email):
    ...
    try:
        ...
        return (
            RetCodes.success.value,
            "User record with email ID  '{0}' successfully fetched from db".format(
                email
            ),
            user,
        )
        ...
        try:
            ...
            for user in userList:
                ...
                if is_tenant_deleted(user.tid):
                    return (
                        RetCodes.sign_in_failed,
                        "Tenant this user belongs to is deleted. Please contact your system administrator",
                        user,
                    )
...
def do_reset_password(id, email, cur_password, new_password, type='reset'):
    ...
    try:
        ...
        try:
            ...
            assert (
                cursor.rowcount == 1
            ), "Failed to find user record. Please contact support"
