	...
	try:
		...
		if (max_yrs_of_exp is not None and min_yrs_of_exp is not None):
			if int(min_yrs_of_exp) == constants.NEW_ENTITY_ID or int(max_yrs_of_exp) == constants.NEW_ENTITY_ID:
				return(RetCodes.empty_ent_attrs_error.value, "Experience field is empty", None)
			if (int(max_yrs_of_exp) < int(min_yrs_of_exp)):
				return(RetCodes.save_ent_error.value, "Maximum years of experience should be more than minimum years of experience ", None)

		if (ctc_max is not None and ctc_min is not None):
			if (ctc_max < ctc_min):
				return(RetCodes.save_ent_error.value, "Maximum CTC cannot be less than minimum CTC", None)
...
def get_resumes_associated_with_job(job_id, cat_status_code=None,sub_cat_status_code=None):
	try:
		...
		query = """select wr_resumes.*,wr_jd_resumes.status ,
					...
					where wr_resumes.is_deleted = %s and wr_resumes.id = wr_jd_resumes.resume_id
...
def get_resumes_not_associated_with_job(job_id, ftsearch, tenant_id):
	try:
		...
		query = """select *,creation_date > NOW() - INTERVAL '1 DAY' as recently_added
					from wr_resumes where is_deleted = False and id not in 
...
# for a given job, email details to all candidates in your tenant
def email_all_candidates(job_id,tenant_id):

	try:
		(retCode,msg,job) = get(job_id)
		(retCode,msg,resumeList) = resumeUtils.list_resumes_by_tenant(tenant_id)

		for resume in resumeList:
			try:
				emailUtils.sendMail(resume.email,job.title, job.details,'plain')
			except Exception as e1:
				_logger.error(e1)

		return (RetCodes.success.value, 'Email sent successfully.', None)

	except Exception as e2:
		_logger.error(e2)
		return (RetCodes.server_error, str(e2), None)
