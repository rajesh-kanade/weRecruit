...
def save_jd(
    ...
    ip_email2=None,
    ip_phone2=None,
    hiring_mgr_name=None,
    hiring_mgr_email=None,
    hiring_mgr_phone=None,
    hr_name=None,
    hr_email=None,
    hr_phone=None,
    ...
    ctc_currency=None,
    fees_percent=None,
    warranty_period_in_months=None,
    max_yrs_of_exp=None,
):
    ...
    try:
        ...
        if max_yrs_of_exp is not None and min_yrs_of_exp is not None:
            if max_yrs_of_exp < min_yrs_of_exp:
                return (
                    RetCodes.save_ent_error.value,
                    "Maximum years of experience can not be less then minimum years of experience .",
                    None,
                )
        ...
        if ctc_max is not None and ctc_min is not None:
            if ctc_max < ctc_min:
                return (
                    RetCodes.save_ent_error.value,
                    "Maximum CTC can not be less then minimum CTC .",
                    None,
                )
        ...
        if int(id) == constants.NEW_ENTITY_ID:
            ...
            if jd_file_name != None:
                _logger.debug(
                    "JD file name is not null so updating the file name and content for job id {0}".format(
                        id
                    )
                )
            ...
            return (
                ...
                "JD {0} updated successfully.".format(id),
                id,
            )
...
def save_header(id, title, details, client):
    ...
    try:
        ...
        assert (
            cursor.rowcount == 1
        ), "assertion failed : {0} Rows returned which is not equal to 1.".format(
            cursor.rowcount
        )
...
def list_jds_by_tenant(
    tenantID, orderBy=None, order=None, statusFilter=None, limit=1000
):
    try:
        db_con = dbUtils.getConnFromPool()
        cursor = dbUtils.getNamedTupleCursor(db_con)

        q1 = """SELECT * FROM wr_jds 
        ...
        return (
            RetCodes.success.value,
            "JD List successfully fetched from db for tenant ID".format(tenantID),
            jdList,
        )
...
def get(id):
    try:
        ...
        return (
            RetCodes.success.value,
            "JD  info for {0} successfully fetched from db".format(id),
            jd,
        )
...
def get_resumes_associated_with_job(job_id):
    try:
        ...
        return (
            RetCodes.success.value,
            "Resumes associated with job JD {0}  fetched successfully.".format(job_id),
            resumeList,
        )
...
def get_resumes_not_associated_with_job(job_id, ftsearch, tenant_id):
    try:
        ...
        return (
            RetCodes.success.value,
            "Resumes not associated with job JD {0}  fetched successfully.".format(
                job_id
            ),
            resumeList,
        )
...
def insert_job_application_status(
    job_id, resume_id, change_date, changed_by, status, notes
):
...
def shortlist(resume_id, jd_id, application_date, status, recruiterid):
    ...
    try:
        ...
        insert_job_application_status(
            jd_id, resume_id, application_date, recruiterid, status, ""
        )
...
def get_job_status_summary(job_id):
    ...
    try:
        ...
        return (
            RetCodes.success.value,
            "Status summary  for job JD {0}  fetched successfully.".format(job_id),
            jobStatusSummaryList,
        )
...
def update_job_stats():
    ...
    try:
        ...
        ), "Failed to fetch application status codes."
